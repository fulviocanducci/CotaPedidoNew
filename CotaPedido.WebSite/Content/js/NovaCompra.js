...
function IncluirProdutoPedido() {
    ...
    if (isNaN(quantidade) || !isNaN(quantidade) && quantidade > 100 || !isNaN(quantidade) && quantidade < 1) {
        erro += '<li>O campo Quantidade é obrigatório e deve estar entre 1 e 100</li>';
    }
    ...
    if (erro != '') {
        $('#modelAlert .modal-body').html('<h3>Os seguintes erros foram encontrados:</h3><ul class="has-error">' + erro + '</ul>');
        $('#modelAlert').modal('show');
    } else {
        produtosInseridos = GerarIndex();
        var templateProduto = '<tr><td>{NomeProduto}<input type="hidden" value="{index}" class="index" disabled><input type="hidden" name="Itens[{index}].NomeProduto" value="{NomeProduto}" /></td><td>{Nomecategoria}<input type="hidden" name="Itens[{index}].Idcategoria" value="{Idcategoria}" /></td><td>{NomeSubcategoria}<input type="hidden" name="Itens[{index}].IdSubcategoria" value="{IdSubcategoria}" /></td><td>{NomeUnidade}<input type="hidden" name="Itens[{index}].IdUnidade" value="{IdUnidade}" /></td><td><input type="text" name="Itens[{index}].Quantidade" value="{Quantidade}" class="input-qty form-control text-center" /></td><td class="text-center content-middle"><a href="#" class="remove_cart" rel="2"><i class="fa fa-trash-o fa-2x"></i></a></td></tr>';
        ...
        $('#produtos').append(templateProduto);
...
$(document).ready(function () {
    ...
    $('#quantidade').mask('9xx', { translation: { 'x': { pattern: /[0-9]/, optional: true } } });
    ...
    $('#novaSubcategoria').focusout(function () {
        var $erro = $('#errorNovaSubcategoria');
        var nome = $(this).val();
        nome = nome.trim();
        if (nome == '') {
